...
function changeLanguage(lang) {
    ...
    elements.forEach(element => {
        ...
        if (lang === 'id') {
            if (langId) {
                element.textContent = langId;
            }
        } else if (lang === 'en') {
            if (langEn) {
                element.textContent = langEn;
            }
        }
    });
}
...
if (localStorage.getItem('language')) {
    ...
    // Defaultkan bahasa ke ID jika belum ada preferensi
    changeLanguage('id');
}

// Menyimpan bahasa yang dipilih di localStorage
languageSwitcher.addEventListener('change', (e) => {
...
footerCopy.innerText = `${String.fromCharCode(169)} ${tahunSekarang} TMSBK. All right reserved`;
